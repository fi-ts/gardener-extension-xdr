...
.PHONY: build
build:
	go build -ldflags $(LD_FLAGS) -tags netgo -o bin/gardener-extension-xdr ./cmd/gardener-extension-xdr
...
docker-image:
	@docker build --no-cache \
		...
		--tag $(IMAGE_PREFIX)/gardener-extension-xdr:$(IMAGE_TAG) \
		--file Dockerfile --memory 6g .

.PHONY: docker-push
docker-push:
	@docker push $(IMAGE_PREFIX)/gardener-extension-xdr:$(IMAGE_TAG)
